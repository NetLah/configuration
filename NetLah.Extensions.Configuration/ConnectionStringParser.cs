...
namespace NetLah.Extensions.Configuration
{
    internal class ConnectionStringParser
    {
        ...
        private readonly Func<string, string> _keyNormalize;

        public ConnectionStringParser(IEnumerable<KeyValuePair<string, string>> configuration, ProviderName providerName, Func<string, string> keyNormalize)
        {
        ...
        internal IEnumerable<ProviderConnectionString> ListConnections()
        {
            var lookup = _configuration
                .Select(c => (normalizedKey: _keyNormalize(c.Key).ToUpperInvariant(), key: _keyNormalize(c.Key), value: c.Value))
        ...
        internal Dictionary<string, ProviderConnectionString> ExpandValue(IEnumerable<ProviderConnectionString> enumerable)
        {
            ...
            while (list.Count > 0)
            {
                ...
                {
                    var entry = list.FirstOrDefault(i => i.Remaining == minRemaining);
                    Resolve(entry);
                }
                ...
                string ExpandMatchEvaluator(Match match)
                {
                    ...
                    if (match.Groups["token"].Value is { } token &&
                        !string.IsNullOrEmpty(token) && resolved.TryGetValue(_keyNormalize(token), out var entry))
                    {
        ...
        {
            var result = new Entry { ConnStr = connStr };

            if (connStr.Raw is { } raw)
            {
                var matches = QuoteTokenRegex.Matches(raw);

                result.Tokens = matches
                    .Select(m => _keyNormalize(m.Groups["token"].Value))
                    .Where(token => !string.IsNullOrEmpty(token))
                    .ToHashSet(DefaultStringComparer);

                result.HasQuotes = matches
